...
# MySQL DB 연결 설정
mysql_conn = mysql.connector.connect(user=db_user, password=db_password, host=db_host, port=db_port, database=db_name)
...
def check_duplicate():
    try:
        ...
        mysql_cursor.execute("SELECT COUNT(*) FROM users WHERE id = %s", (id_to_check,))
...
def sign_up():
    try:
        ...
        # 데이터베이스에 저장
        mysql_cursor.execute("INSERT INTO users (id, password, gender, age) VALUES (%s, %s, %s, %s)",(id, password, gender, age))
        mysql_conn.commit()

        return jsonify({'status': 'success'})

    except mysql.connector.Error as e:
        return jsonify({'status': 'error', 'message': f'MySQL 에러: {str(e)}'})
    except Exception as e:
        return jsonify({'status': 'error', 'message': f'예기치 않은 오류 발생: {str(e)}'})
...
def login():
    ...
    mysql_cursor.execute(query, (username, password))
...
def get_attraction_name():
    ...
    if selected_location_name is not None:
        query = "SELECT ATTRACTION_NAME FROM SPOT WHERE LOCATION_ID = %s"

        mysql_cursor.execute(query, (selected_location_name,))
...
def get_recommendations_for_main(user_gender, user_age, user_relationship, user_destination):
    ...
    surprise_data = Dataset.load_from_df(df_mysql[['ID', '관광지', '평점']], reader)
    ...
    user_id = df_mysql['ID'].max() + 1
    user_recommendations = []

    for attraction_id in df_mysql['관광지'].unique():
        if not df_mysql[(df_mysql['ID'] == user_id) & (df_mysql['관광지'] == attraction_id)].empty:
            continue
